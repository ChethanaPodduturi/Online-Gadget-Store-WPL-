...
//var_dump($memcache->fetchAll());
//set the key then check the cache
$key = "SELECT * FROM products where product_type='Laptop'";
$get_result = $memcache->get(Array ($key));

if ($get_result) {
	while($obj = $get_result->fetch_object())
        {
...
else 
{
 // Run the query and get the data from the database then cache it
    ...
    if(!$product)
	{
	$results = $mysqli->query("SELECT * FROM products where product_type='Laptop' ORDER BY id ASC");
    if ($results) { 
		...
		$memcache->set($key, $obj, false, 1);		// Store the result of the query for 20 seconds
		require_once "lib/nusoap.php";
$client = new nusoap_client("http://localhost/tool-shop/server.php");
...
if ($client->fault) {
	...
	<button class="outofform" onClick="location.href='lapIops.php'">Reload Page</button>
	...
	// SQL query
	$strSQL = "SELECT * FROM review where Product_code='Laptop'";
